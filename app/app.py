...
class VideoTransformer(VideoTransformerBase):
    ...
    def transform(self, frame):
        ...
        curr_time = time.time()
        fps = 1.0 / (curr_time - self.prev_time) if (curr_time - self.prev_time) > 0 else 0
        ...
        st.session_state.latest_counts = counts
        st.session_state.latest_has_violation = has_violation
...
if "latest_counts" not in st.session_state:
    st.session_state.latest_counts = {name: 0 for name in CLASS_NAMES}
if "latest_has_violation" not in st.session_state:
    st.session_state.latest_has_violation = False
...
if "Image Upload" in mode:
    ...
    if webrtc_ctx.state.playing:
        ...
        while webrtc_ctx.state.playing:
            counts = st.session_state.latest_counts
            has_violation = st.session_state.latest_has_violation
        ...
        st.info("Click 'START' on the video component above to begin the webcam stream.")
